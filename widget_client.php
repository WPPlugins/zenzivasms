<script type='text/javascript' src='<?php bloginfo('url');?>/wp-content/plugins/zenzivasms/captcha/latest-jquery/jquery-1.5.1.min.js'></script>
<script type='text/javascript' src='<?php bloginfo('url');?>/wp-content/plugins/zenzivasms/captcha/latest-jquery-ui/jquery-ui.min.js'></script>
<script type='text/javascript' src='<?php bloginfo('url');?>/wp-content/plugins/zenzivasms/captcha/jquery.captcha.js'></script>
...
<script type="text/javascript" charset="utf-8">
	...
	var formId = "send_sms_form" /* id of your form */
	var items = Array("pensil", "gunting", "jam", "hati", "buku"); 
	var style = 'margin-top: 20px; padding: 0px;';
	...
	$(function() {
		$(".ajax-fc-container").captcha({ /* in this line note that ajax-fc-container is a class and we reference it with .(dot), if you want to change class to id, don't forget to replace the dot with # sign and also to describe it in css file. */
			captchaDir: "<?php echo bloginfo('url')?>/wp-content/plugins/zenzivasms/captcha/",
			url: "<?php echo bloginfo('url')?>/wp-content/plugins/zenzivasms/captcha/captcha.php",
			text : "Tarik <span>gunting</span> kedalam lingkaran."
		}); 
...
<?php
	...
	if ( $_POST['sms_send_'.$sms_section_from] && isset($_POST['captcha']) ){
